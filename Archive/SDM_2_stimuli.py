...
for room in rooms:
    RIR_dir = os.path.join(root_dir, "Impulses", room.name, "Eigenmike\\SDM\\normalised")
    for method in ambisonics:
        ...
        for i in range(len(stimuli_pos)):
            ...
            RIR = os.path.join(RIR_dir, room.name + "_" + "Eigenmike" + "_" + str(stimuli_pos[i]) + "_Impulse.wav")
        ...
        for ind, channel in enumerate(output):
            ...
            if mode is "Stimuli":
                ...
                filename = os.path.join(output_dir, room.name + "_" + method.name + "_Noise_" + str(ind) + ".wav")
            elif mode is "Dirac":
